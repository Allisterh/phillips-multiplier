...
__NOTE__: The following code cannot run simply as-is since it relies on sourcing several auxiliary functions. The easiest way to use this notebook is to clone the entire R project folder which is located here, open the project on Rstudio, and knit this R markdown notebook.
...
vParB       <- seq(-10, 5, by = 0.5) # grid of \beta parameters to test
...
p_pi <- 
  results %>% 
  ggplot(aes(h, vBetaPi)) +
  geom_line(size = 1, color = "black") +
  geom_ribbon(
    aes(ymin = vLowerPI, ymax = vUpperPI),
    fill = "#05827e",
    alpha = 0.2
  ) +
  geom_hline(aes(yintercept = 0), linetype = 2) +
...
p_ur <- 
  results %>% 
  ggplot(aes(h, vBetaUr)) +
  geom_line(size = 1, color = "black") +
  geom_ribbon(
    aes(ymin = vLowerUR, ymax = vUpperUR),
    fill = "#05827e",
    alpha = 0.2
  ) +
  geom_hline(aes(yintercept = 0), linetype = 2) +
